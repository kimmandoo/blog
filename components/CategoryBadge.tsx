...
export function CategoryBadge({ category, index = 0, size = 'md', clickable = true }: CategoryBadgeProps) {
  ...
  const badge = (
    <span
      className={`category-badge inline-flex items-center font-semibold rounded-xl ${colorClass} ${sizeClasses[size]} backdrop-blur-sm border border-white/20 dark:border-white/10 shadow-sm hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-300 ease-out cursor-pointer`}
    >
