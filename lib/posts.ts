...
export function getSortedPostsData(): PostData[] {
  ...
  const allPostsData = fileNames
    ...
    // Filter out draft posts and LeetCode posts
    .filter(post => !post.draft && post.category !== 'LeetCode');
...
export function getAllPostSlugs() {
  ...
  return fileNames
    .filter(fileName => {
      // Check if post is draft or LeetCode post
      const fullPath = path.join(postsDirectory, fileName);
      const fileContents = fs.readFileSync(fullPath, 'utf8');
      const matterResult = matter(fileContents);
      return !matterResult.data.draft && matterResult.data.category !== 'LeetCode';
    })
    .map(fileName => {
      return {
