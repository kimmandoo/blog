...
const blockchainDirectory = path.join(process.cwd(), 'blockchain');
const blockchainPostsDirectory = path.join(blockchainDirectory, 'posts');
export const blockchainImagesDirectory = path.join(blockchainDirectory, 'images');
...
export function getSortedBlockchainData(): BlockchainData[] {
  // Check for posts directory first, fallback to root blockchain directory
  const searchDirectory = fs.existsSync(blockchainPostsDirectory) 
    ? blockchainPostsDirectory 
    : blockchainDirectory;
  ...
  const allData = fileNames
    .map(fileName => {
      ...
      return {
        ...
        readingTime,
        ...(matterResult.data as Omit<BlockchainData, 'slug' | 'title' | 'date' | 'excerpt' | 'category' | 'tags' | 'draft' | 'readingTime'>),
      };
    })
...
export function getAllBlockchainSlugs() {
  // Check for posts directory first, fallback to root blockchain directory
  const searchDirectory = fs.existsSync(blockchainPostsDirectory) 
    ? blockchainPostsDirectory 
    : blockchainDirectory;
...
export async function getBlockchainData(slug: string): Promise<BlockchainData> {
  ...
  return {
    ...
    toc,
    readingTime,
    ...(matterResult.data as Omit<BlockchainData, 'slug' | 'title' | 'date' | 'content' | 'excerpt' | 'category' | 'tags' | 'draft' | 'toc' | 'readingTime'>),
  };
}
...
export function getAllBlockchainCategories(): string[] {
  const items = getSortedBlockchainData();
...
export function getAllBlockchainTags(): string[] {
  const items = getSortedBlockchainData();
