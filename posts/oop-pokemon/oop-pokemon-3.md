...
`RestoreHP`, `Paralyze`, `AttackStatChange`, `Faint`도 같은 방식으로 읽힌다. 결국은 "누구에게 붙일 것인가?"만 정해지면 된다.
...
그래서 Effect는 변경의 종류를 책임지고, 숫자 계산은 `INumber`가 책임지는 편이 깔끔하다. 이렇게 해두면 `Damage`는 그대로 두고 바깥에서 숫자 공급자만 갈아 끼우면 된다. 고정 40 데미지든, 타입 상성과 STAB(자기 타입 보정)가 반영된 계산식이든, 이번 시도의 데미지 결과를 바탕으로 계산하는 반동이든 전부 같은 `Damage`가 처리할 수 있다.
...
즉 무엇을 바꿀지와 얼마나 바꿀지를 분리해두면 기술이 늘어날수록 관리가 쉬워진다.
...
이제 마무리 겸 기술을 다시 읽어보자. `섀도볼`, `돌진`, `흡혈`, `칼춤`, `스텔스록`은 이름은 달라도 결국 데미지, 상태이상, 회복, 랭크 변경, 필드 변경 같은 익숙한 Effect 조각의 조합으로 읽힌다. 달라지는 건 이 조각들의 순서와 조건, 그리고 숫자 계산 방식이다.
