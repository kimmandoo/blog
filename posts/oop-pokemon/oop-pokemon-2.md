...
이 질문들을 `if`문 덩어리로 계속 붙이는 대신 이름 붙은 조건으로 잘게 쪼개서 조합할 수 있으면 구조가 훨씬 덜 무너질 것 같다. 이번 글은 그 이야기를 해보려 한다.
...
이렇게 해두면 "공격자가 마비 상태인가?"와 "방어자가 땅 타입인가?"는 사실상 같은 종류의 검사다. 둘 다 Battler 조건을 평가하는 것이고, 달라지는 건 어떤 대상을 고르느냐뿐이다.
