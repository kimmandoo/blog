...
즉 `Trigger`는 새로운 기술 종류라기보다 이미 일어난 일 뒤에 붙는 후속 반응이라고 보면 된다.
...
이렇게 하면 책임 범위를 나누기 쉬워진다.

지루할 수 있겠지만 `정전기`를 예시로 들어보겠다. 플레이어 입장에서는 "접촉하면 가끔 마비 걸리는 특성" 한 줄이지만, 시스템 입장에서는 접촉 Event를 받고 조건을 다시 확인한 뒤 반응한다.
...
이 구조가 필요한 이유는 Trigger도 또 다른 Event를 만들 수 있기 때문이다. 예를 들어 `까칠한피부`로 사용자가 쓰러질 수도 있다. 그러면 거기서 다시 쓰러짐 관련 Trigger나 후속 처리로 이어질 수 있다.

정리하면 `Trigger`는 "반응 규칙"을 위한 자리다. 기술은 자기 `Effect`를 실행하고, 배틀은 `Event`를 남기고, 특성이나 도구 같은 다른 규칙은 `Trigger`로 반응한다.
