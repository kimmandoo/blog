...
지난 글에서는 `Effect`를 봤다. 조건을 통과한 뒤 실제로 HP가 깎이고, 상태이상이 걸리고, 날씨가 바뀌는 그 변화 말이다.
...
즉 시스템은 단순히 "HP가 바뀌었다"보다 한 단계 더 구체적인 "방금 배틀에서 실제로 벌어진 일"을 알아야 한다.
...
`DamageDealt`는 누가 누구에게 얼마의 데미지를 줬는지를 담고, `ContactHappened`는 접촉이 있었는지를 담는다. `StatusInflicted`는 누가 누구에게 어떤 상태이상을 걸었는지를 담는다. `StatChanged`는 공격이나 방어 랭크가 어떻게 바뀌었는지를 담고, `HPRecovered`는 누가 얼마만큼 회복했는지를 담는다. `BattlerFainted`는 어느 포켓몬이 쓰러졌는지라는 사실을 담는다.
...
다만 이걸 너무 딱딱한 1:1 대응으로 볼 필요는 없다. 하나의 Effect가 Event를 여러 개 남길 수도 있고, `NoEffect`처럼 아무 Event도 남기지 않을 수도 있다. "상태를 바꾼다"와 "무슨 일이 일어났는지 남긴다"가 분리된다는게 핵심이다.
...
다른 규칙이 그 Event를 보고 반응할 수 있다는 점이 Event에서 가장 중요한 부분일 것 같다.

예를 들어 상대를 접촉 기술로 때렸다고 해보자. 그러면 데미지가 들어갔다는 사실은 `DamageDealt`로 남고, 접촉이 있었다는 사실은 `ContactHappened`로 따로 남길 수 있다. 또 쓰러짐은 후속 판정 뒤 `BattlerFainted`로 기록될 수 있다. 다른 규칙은 이 Event들을 보고 각자 반응한다.

```mermaid
flowchart LR
    A[DamageDealt Event] --> B[로그 출력]
    A --> C[후속 판정]
    C --> D[BattlerFainted Event]
    E[ContactHappened Event] --> F[접촉 반응 검사]
    D --> G[교체 단계 열기]
```

예를 들어 `볼트태클`이 맞은 뒤를 생각해보면 흐름은 대충 이럴 것이다.

```text
1. 상대에게 데미지가 들어간다
2. DamageDealt Event가 남는다
3. 접촉 기술이었다면 ContactHappened Event도 남는다
4. 후속 판정에서 상대가 쓰러졌다면 BattlerFainted Event가 남는다
5. 추가 효과가 붙었으면 StatusInflicted Event가 남는다
6. 마지막으로 사용자 반동이 들어간다
7. 그 반동도 또 하나의 DamageDealt Event로 남는다
8. 내가 쓰러질 수도 있다
```
